...
class model:
    ...
    def xload_variable_config(self):
        ...
        if self.target=="sentence_length":
            self.config=variables[variables['treatment_violent_lr'].isin(['cont_wnans','one_hot','transform',binary_wnans'])
        else:
            #A column giving default treatment of columns
            #if the columns don't have a treatment type then they won't be picked up
            self.config=variables[variables[self.treatment].isin(['cont_wnans','one_hot','transform','binary_wnans'])]
        
    def xpreprocess(self):
