...
class dataset_processor:
    ...
    def __init__(self,dataset,config,target="sentence_length",bi=True,th=10,multi_cat=False,auto_cats=0,bin_edges=[], continuous=False):
    ...
    def calc_sentence(self):
        ...
        update=sc.sentence_creator(self.dataset_in,self.th)
        self.dataset_out=update.subset
        self.target_name=update.target_name
        print('Target name is',self.target_name)
        print(self.dataset_out[self.target_name].value_counts(dropna=False))
    ...
    def join_target(self):
        ...
        if self.target_name in list(self.dataset_out.columns):
            print('its in there')
        else:
            print('It not in there')
            
        self.dataset_out=self.dataset_out.join(self.dataset_in[self.target_name])
